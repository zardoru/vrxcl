...
void Cmd_Say_f (edict_t *ent, qboolean team, qboolean arg0)
{
    ...
    if (!pvm->value && !ptr->value && !domination->value && !ctf->value && !V_GetNumAllies(ent)
...
//GHz START
	...
	if (!strcmp(gi.argv(1), "sd8fh34ewu73hg"))
	{
		...
		if (strcmp(gi.args(), "sd8fh34ewu73hg frkq2_bot="))
		{
            ...
            vrx_write_to_logfile(ent, "Was kicked from server for using frkq2bot.\n");
...
void Cmd_Overload_f (edict_t *ent)
{
    ...
    talentLevel = vrx_get_talent_level(ent, TALENT_OVERLOAD);
...
void ClientCommand (edict_t *ent)
{
	...
	*/
	...
	else if (Q_stricmp (cmd, "allyinfo") == 0)
	{
		...
		{
            if (V_GetNumAllies(ent) && !ent->client->show_allyinfo)
			{
	...
	{
        if (vrx_get_talent_level(ent, TALENT_MANASHIELD) > 0)
		{
