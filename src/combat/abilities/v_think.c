    ...
    if (ent->client && vrx_get_talent_slot(ent, TALENT_BASIC_AMMO_REGEN) != -1) {
            ...
            && !(ent->client->buttons & BUTTON_ATTACK)) // don't regen ammo while firing
        {
            ...
            switch (ent->client->ammo_index) {
                case 24:
                    V_GiveAmmoClip(ent, 1.0, AMMO_ROCKETS);
                    break;    //rocket_index
                case 21:
                    V_GiveAmmoClip(ent, 1.0, AMMO_BULLETS);
                    break;    //bullet_index
                case 22:
                    V_GiveAmmoClip(ent, 1.0, AMMO_CELLS);
                    break;    //cell_index
                case 23:
                    V_GiveAmmoClip(ent, 1.0, AMMO_GRENADES);
                    break;    //grenade_index
                case 20:
                    V_GiveAmmoClip(ent, 1.0, AMMO_SHELLS);
                    break;    //shell_index
                case 25:
                    V_GiveAmmoClip(ent, 1.0, AMMO_SLUGS);
                    break;    //slug_index
                default: //do nothing
                    break;
            }
